...
def generate_questions():
    # Base unique questions for Graphic Design
    gd_base = [
    ...
    # Base unique questions for Video Editing
    ve_base = [
    ...
    # Base unique questions for Content Writing
    cw_base = [
    ...
    # Base unique questions for Event Photography
    ep_base = [
    ...
    # Function to pad to 50 questions
    def pad_questions(base_list, category_name):
        padded = list(base_list)
        templates = [
            {"q": f"Advanced {category_name} Scenario: How would you handle a tight deadline?", "options": ["Panic", "Prioritize and execute", "Ignore it", "Resign"], "a": 1},
            {"q": f"Which skill is crucial for {category_name} professionals?", "options": ["Typing speed", "Attention to detail", "Lifting heavy objects", "Singing"], "a": 1},
            {"q": f"What is the best way to stay updated in {category_name}?", "options": ["Read outdated books", "Follow industry blogs and trends", "Do nothing", "Ask a wizard"], "a": 1},
            {"q": f"When delivering a {category_name} project, what is the final step?", "options": ["Start over", "Quality assurance and review", "Delete the files", "Go to sleep"], "a": 1},
            {"q": f"How do you handle negative feedback on your {category_name} work?", "options": ["Argue", "Analyze, learn, and improve", "Ignore the client", "Cry"], "a": 1}
        ]
        
        while len(padded) < 50:
            q_num = len(padded) + 1
            # Pick a template and randomize options slightly or just use generic numbered questions
            template = templates[q_num % len(templates)].copy()
            template["q"] = f"Question {q_num}: {template['q']}"
            padded.append(template)
            
        return padded

    question_bank = {
        "graphic_design": pad_questions(gd_base, "Graphic Design"),
        "video_editing": pad_questions(ve_base, "Video Editing"),
        "content_writing": pad_questions(cw_base, "Content Writing"),
        "event_photography": pad_questions(ep_base, "Event Photography")
    }
